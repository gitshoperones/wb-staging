@php
	$layout = (request()->vendor_id) ? 'public' : 'dashboard';
@endphp
...
@section('content')
<section class="wb-bg-grey section-postJob content">
	<div class="container-fluid">
		<div class="wb-tab-job">
			<form action="{{ url('/dashboard/job-posts') }}" method="post"
						...
						if(this.type == 'address') {
							...
							this.opts.split(',').map((opt, i) => {
								multi_options += `<li>
									...
									name="fields[${counter}][val][]"
									value="${opt}" />
									<label for="multi-${counter}${i}" style="text-transform: capitalize; margin: 0;">${opt}</label>
								</li>`
							})
