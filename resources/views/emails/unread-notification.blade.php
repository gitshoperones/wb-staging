...
<head>
	...
	<title>wedBooker - unread notifications</title>
	<style>
		/* cyrillic-ext */
		@font-face {
			font-family: 'Ubuntu';
...
<body style="background-color: #e7d8d1;padding: 60px 15px;text-align: center;font-weight: 300;font-family: 'Ubuntu', sans-serif;line-height: 1.6em;">
	<table class="container header" style="padding: 15px; max-width: 600px; margin: auto;background-color: #fff;width: 100%;margin: auto;-webkit-box-shadow: 0 0 3px 0 rgba(0,0,0,.1);box-shadow: 0 0 3px 0 rgba(0,0,0,.1);border-bottom: 1px solid #f1f1f1;">
		...
		<tr>
			<td style="text-align: left;">
				<p style="font-family: 'Ubuntu', sans-serif;text-decoration: none;color: #353554; font-size: 14px;">
                    Dear @if($user->account == 'vendor') {{ ($user->vendorProfile->business_name) ? $user->vendorProfile->business_name : 'Business Owner' }} @elseif($user->account == 'couple') {{ ($user->coupleProfile()->title) ? $user->coupleProfile()->title : 'Couples' }} @endif
                    <br>
                    <br>
		...
		<tr>
			<td style="text-align: left;">
				<a href="{{ url('login') }}" class="btn orange" style="font-family: 'Ubuntu', sans-serif;text-decoration: none;text-transform: uppercase;font-size: 14px;font-weight: 300;border-radius: 3px;padding: 11px 48px;background: #fe5e44;display: inline-block;color: #fff;">Login Now
				</a>
			</td>
		</tr>
	</table>
